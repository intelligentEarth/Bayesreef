...
class MCMC():
    ...
    def plot_results(self, pos_m, pos_ax, pos_ay, pos_sed1, pos_sed2, pos_sed3, pos_sed4, pos_flow1, pos_flow2, pos_flow3, pos_flow4, burn):
        ...
        plt.hist(pos_m, bins=25, alpha=0.5, facecolor='coral', normed=True)
        plt.plot(mspace,pdf_m,label='Best fit',color='r',linestyle='--')
        plt.grid(True)
        plt.title(' Malthusian Parameter', size=self.font+2)
        plt.ylabel('Frequency', size=self.font+1)
        plt.xlabel(r'$\varepsilon$', size=self.font+1)
        plt.savefig('%s/malthus.png'% (self.filename), dpi=300)
        plt.clf()
        ...
        ax.set_title('Community Interaction Matrix Parameters', fontsize= self.font+2, y=1.02)
        ax1 = fig.add_subplot(211)
        ax1.set_facecolor('#f2f2f3')
        ax1.hist(pos_ax, bins=25, alpha=0.5, facecolor='steelblue', normed=True)
        ax1.plot(a1space,pdf_a1,label='Best fit',color='r',linestyle='--')
        ax1.grid(True)
        ax1.set_ylabel('Frequency',size=self.font+1)
        ax1.set_xlabel(r'Main diagonal value ($\alpha_{ii}$)', size=self.font+1)
        ax2 = fig.add_subplot(212)
        ax2.set_facecolor('#f2f2f3')
        ax2.hist(pos_ay, bins=25, alpha=0.5, facecolor='mediumaquamarine', normed=True)
        ax2.plot(a2space,pdf_a2,label='Best fit',color='r',linestyle='--')
        ax2.grid(True)
        ax2.set_ylabel('Frequency',size=self.font+1)
        ax2.set_xlabel(r'Super- and sub-diagonal value ($\alpha_{i,i+1}$ and $\alpha_{i+1,i}$)',size=self.font+1)
        fig.tight_layout()
        fig.subplots_adjust(top=0.88)
        plt.savefig('%s/comm_matrix.pdf' % (self.filename), dpi=300)
        plt.clf()
        ...
        if ((self.sedsim != False)):
            for a in range(0,self.communities):
                ...
                ax.fill_betweenx(cy,c_lb,c_ub, facecolor='mediumaquamarine', alpha=0.4, label=None)
                ...
                plt.ylim(0.,1.1)
                lgd = plt.legend(frameon=False, prop={'size':self.font+1}, bbox_to_anchor = (1.35,0.22))
                plt.savefig('%s/sediment_response_%s.pdf' % (self.filename, a+1), bbox_extra_artists=(lgd,),bbox_inches='tight',dpi=300)
                plt.clf()
        ...
        if (self.flowsim != False):
            for a in range(0,self.communities):
                ...
                ax.fill_betweenx(cy,c_lb,c_ub, facecolor='mediumaquamarine',alpha=0.4, label=None)#color='coral',
                plt.title('Hydrodynamic energy exposure threshold function\n(%s assemblage)' % (a_labels[a]), size=self.font+2, y=1.06)
                plt.ylabel('Proportion of maximum growth rate [%]', size=self.font+1)
                plt.xlabel('Fluid flow [m/sec]', size=self.font+1)
                plt.ylim(0.,1.1)
                lgd = plt.legend(frameon=False, prop={'size':self.font+1}, bbox_to_anchor = (1.35,0.25))
                plt.savefig('%s/flow_response_%s.pdf' % (self.filename, a+1),  bbox_extra_artists=(lgd,), bbox_inches='tight',dpi=300)
                plt.clf()
    ...
    def save_core(self,reef,naccept):
        ...
        reef.plot.speciesDepth(colors=colors, size=(8,4), font=8, dpi=200, fname =('%s/b_popd_%s.pdf' % (path,naccept)))
        reef.plot.speciesTime(colors=colors, size=(8,4), font=8, dpi=200,fname=('%s/c_popt_%s.pdf' % (path,naccept)))
        reef.plot.accomodationTime(size=(8,4), font=8, dpi=200, fname =('%s/d_acct_%s.pdf' % (path,naccept)))
    ...
    def sampler(self):
        ...
        cm_ax = pos_ax[0] = np.random.uniform(-0.5,0.)
        cm_ay = pos_ay[0] = np.random.uniform(-0.5,0.)
        m = pos_m[0] = np.random.uniform(0.,0.5)
        ...
        # Higher acceptance rate with smaller steps?
        step_m = 0.01 # 0.05
        step_a = 0.01 # 0.001
        step_sed = 0.001 # 0.001 
        step_flow = 0.02 # 0.001
        step_eta = 0.1 # 0.01
        ...
        lgd = ax_append.legend(frameon=False, prop={'size':self.font+1})
        final_fig.savefig('%s/proposals.pdf'% (self.filename), extra_artists = (lgd,),bbox_inches='tight',dpi=300)
        plt.clf()
        ...
        plt.plot(x_rmse,pos_rmse,'-',color='coral',label='RMSE')
        ...
        plt.savefig('%s/rmse_evolution.pdf' % (self.filename), dpi=300)
        plt.clf()
...
def main():
    ...
    samples = 10000
    description = 'RKF45,unconstrained, 10000s, smaller m,aij range'
    nCommunities = 3
    simtime = 8500
    ...
    plt.plot(fx_mu,x_data, label='Pred. (mean)')
    plt.plot(fx_low, x_data, label='Pred. (5th %ile)')
    plt.plot(fx_high,x_data, label='Pred. (95th %ile)')

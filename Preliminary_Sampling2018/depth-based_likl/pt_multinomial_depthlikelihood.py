...
class MCMC():
    ...
    def run_Model_(self, reef, input_vector):
...
def main():
    ...
    if problem ==1:
    	...
    	datafile = 'data_/synth_core.txt'
    	core_depths = np.genfromtxt(datafile, usecols=(0), unpack = True) 
    	core_data = np.loadtxt('data_/synth_core_bi.txt')

        true_vec_parameters = np.loadtxt('data_/true_values.txt')
    	...
    	datafile = 'data_/hi3.txt'
    	core_depths = np.genfromtxt(datafile, usecols=(0), unpack = True) 
    	core_data = np.loadtxt('data_/hi3_binary.txt')

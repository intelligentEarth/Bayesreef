...
class MCMC():
    ...
    def plotFunctions(self, fname, v1, likelihood, diff, rmse):
        ...
        plt.savefig('%s/1d_surface_plot.png'% (fname), bbox_inches='tight', dpi=300, transparent=False)
        plt.clf()
        ...
        plt.savefig('%s/diff_evolution.png' % (self.filename), bbox_inches='tight',dpi=300,transparent=False)
        plt.clf()
        ...
        plt.xlabel('%s' % self.var1_title)
        plt.xlim(X.min(), X.max())
        plt.savefig('%s/rmse_evolution.png' % (self.filename), bbox_inches='tight',dpi=300,transparent=False)
        plt.clf()

#!/usr/bin/env python
#Title           :multinom_mcmc_t_constrained.py
...
class MCMC():
    ...
    def sampler(self):
        ...
        flow1[self.assemblage-1] = np.random.uniform(flowlim[0], flow2[self.assemblage-1])
        flow2[self.assemblage-1] = np.random.uniform(flow1[self.assemblage-1], flow3[self.assemblage-1])
        flow3[self.assemblage-1] = np.random.uniform(flow2[self.assemblage-1], flow4[self.assemblage-1])
        ...
        for i in range(samples - 1):
            ...
            print 'u', u, 'and mh_probability', mh_prob, 'and q ratio', q_ratio
        ...
        finalfig.savefig('%s/proposals_t.png'% (self.filename),bbox_inches='tight',dpi=200,transparent=False)
        plt.close()

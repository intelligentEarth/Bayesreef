...
def main():
    ...
    if problem ==1:
    	...
    	simtime = 8500
    	timestep = np.arange(0,simtime+1,50)
    	xmlinput = 'input_oti5_threeasembledges.xml'
    	datafile = 'data/synth_core.txt'
    	core_depths = np.genfromtxt(datafile, usecols=(0), unpack = True) 
    	core_data = np.loadtxt('data/synth_core_bi.txt')
    ...
    run_nb = 0
    while os.path.exists('results_final%s' % (run_nb)):
        run_nb+=1
    if not os.path.exists('results_final%s' % (run_nb)):
        os.makedirs('results_final%s' % (run_nb))
    filename = ('results_final%s' % (run_nb))
